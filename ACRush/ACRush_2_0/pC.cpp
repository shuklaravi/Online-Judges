...
//	freopen("C-small-attempt1.in","r",stdin);freopen("C-small-attempt1.ans","w",stdout);
//	freopen("C-large.in","r",stdin);freopen("C-large.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int i=0;i<n;i++)
		{
			...
			if (p1>=0) addedge(i,p1);
			if (p2>=0) addedge(i,p2);
			if (p3>=0) addedge(i,p3);
		}
		int ret=solve();
		printf(" %d\n",ret);
		fflush(stdout);

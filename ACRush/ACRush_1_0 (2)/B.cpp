...
int ID(int x,int y,int s,int t)
{
	return ((x*100+y)*100+s)*100+t;
}
void addnode(int x,int y,int s,int t,int d)
{
	if (D[x][y][s][t]>=0 && D[x][y][s][t]<=d) return;
	D[x][y][s][t]=d;
	M.insert(MP(d,ID(x,y,s,t)));
}
int main()
{

...
int maxEnergy(vector <int> weight)
{
	int res=0,i;
    vector<int> comb;
    FOR(i,1,weight.size()-1) PB(comb,i);
...
//        print(comb);
        ...
        int larger;
        int temp = 0;
        bool used[weight.size()];
        MEM(used,0);

        REP(i,comb.size())
        {
            used[comb[i]] = true;
  //          cout<<"used "<<i<<" "<<comb[i]<<endl;
            lesser = comb[i]-1;
            while(used[lesser]) lesser--;
            larger = comb[i]+1;
            while(used[larger]) larger++;
            temp += (weight[lesser]*weight[larger]);
            cout<<lesser<<" "<<larger<<endl;
        }
...
// BEGIN CUT HERE
	...
	void test_case_1() { int Arr0[] = {897,802,765,992,1,521,220,380,729,969}; vector <int> Arg0(Arr0, Arr0 + (sizeof(Arr0) / sizeof(Arr0[0]))); int Arg1 = 10400; verify_case(1, Arg1, maxEnergy(Arg0)); }

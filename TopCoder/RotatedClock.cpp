...
string getEarliest(int h, int m)
{
    ...
    while(tries--)
    {
        ...
        m+=30;
        m%=360;
        double dd=m/(double)360.0;

        if(std::fabs(dd - frac) < std::numeric_limits<double>::epsilon()) {flag=1;
            cout<<h<<" "<<m<<endl;
            hh=h/30;
            mm=m/6;
            if(hh<hh_) hh_=hh;
    ...
    sprintf(sss,"%d",hh_);
    sprintf(ssss,"%d",mm_);
//    char *s=itoa1(hh_,sss,10);
//    char *ss=itoa(mm_,ssss,10);
    cout<<sss<<" "<<ssss<<endl;
    if(flag)
	{
	    string pp(sss);

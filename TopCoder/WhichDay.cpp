...
string getDay(vector <string> notOnThisDay)
{
	...
	int i;
	map<string, bool> m;
	m["Sunday"]=1; m["Monday"]=1; m["Tuesday"]=1; m["Wednesday"]=1; m["Thursday"]=1; m["Friday"]=1; m["Saturday"]=1;
	REP(i,notOnThisDay.size())
	{
		m[notOnThisDay[i]]=0;
	}
	if(m["Sunday"]==1) return "Sunday";
	if(m["Monday"]==1) return "Monday";
	if(m["Tuesday"]==1) return "Tuesday";
	if(m["Wednesday"]==1) return "Wednesday";
	if(m["Thursday"]==1) return "Thursday";
	if(m["Friday"]==1) return "Friday";
	return "Saturday";
}

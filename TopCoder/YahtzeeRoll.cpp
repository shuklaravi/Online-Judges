...
public:
    ...
    bool chkY()
    {
        int i;
        REP(i,6)
        if(arr[i]!=5 || arr[i]!=1) return false;
        return true;
    }
    int nn()
    {
        ...
        c=0;
        auto int arjjr[3];
        REP(i,6) if(arr[i]==6) return 1;
        REP(i,6) if(arr[i]==1) return 2;
        REP(i,6)
        {
            c=0;
            FOR(j,i,i+4)
...
double bestChoice(vector <int> d)
{
    dice=d;
	double res,now,then;
	MEM(arr,0);
	...
	if(flag==4) now=25.0;
	int i,j,k,p,q;
	if(chkY() && flag!=1) now=now>8.333333333333334?8.333333333333334:now;
	if(chkLS() && flag!=2) now=now>
	return res;
}

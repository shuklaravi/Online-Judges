...
int fewestMoves(vector <int> board, int k)
{
    ...
    while(!q.empty() )
    {
        ...
        q.pop();
        vector<int> ttt=t.first;
        sort(ALL(ttt));
        if( t.first==ttt ) return t.second;
        REP(i,t.first.size()-k+1) {
            vector<int> tt=t.first;
            reverse(tt.begin()+i, tt.begin()+(i+k));
            if(m[tt]==1) continue;
            q.push( MP(tt, t.second+1));
            m[tt]=1;
        }
    }
	return -1;

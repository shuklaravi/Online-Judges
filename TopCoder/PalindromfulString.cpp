...
{
    long long ret=0;
    int arr[str.length()+1][str.length()+1];
    int i,j,k,p,q;
    MEM(arr,0);
    REP(i,str.length())
        arr[i][i]=1;
    ...
    {
        for(i=0,j=k;i<str.length()-k && j<str.length();i++,j++)
        {
            if( str[i]==str[j] && arr[i-1][j-1]!=0) arr[i][j]=2+arr[i-1][j-1];
            if( str[i]==str[j] && arr[i-1][j-1]==0 ) arr[i][j]=1;
            if(i==1 && j==3) {
                cout<<i-1<<" "<<j-1<<" "<<endl;
                cout<<".................. "<<arr[i][j]<<" "<<arr[i-1][j-1]<<" "<<str[i]<<" "<<str[j]<<endl;
            }
        }
    }
    REP(i,str.length())
    ...
    cout<<str<<" has "<< ret<<" M= "<<M<<endl;
    if(ret>=K)
    return 1;
}
void rec(int x)
{
    cout<<"now in rec "<<x<<endl;
    getchar();
    int i;
    if(x==N)
    res+=func( ss );

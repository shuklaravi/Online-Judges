...
{
	short int lookup[] = { 0, 8,  4, 12, 2, 10, 6, 14 , 1, 9,  5, 13, 3, 11, 7, 15 };
    short int bdata=32928;
    char* first=(char*)bdata;
    short int bData=first;
  //  cout<<sizeof(bData);
    //short int ret_val = (short int)(((lookup[(bData & 0x0F)]) << 4) + lookup[((bData & 0xF0) >> 4)]);
    //cout<<ret_val;
//    bData=((char*)bdata)[1];
  //  ret_val = (short int)(((lookup[(bData & 0x0F)]) << 4) + lookup[((bData & 0xF0) >> 4)]);
    //cout<<ret_val;
        
	
	
	string res,res1,ress;
	int i,j,k;
	for(i=1;i<=s.length();i++)
	{
		string append=s.substr(s.length()-i,i);
		res1=s+append;
		cout<<res1<<endl;
		for(k=i,j=0;j<s.length();j++,k++) {
			if(res1[k]!=s[j]) break;
		}
		if(j==s.length()) {ress=res1; break;}

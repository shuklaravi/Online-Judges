    ...
    // $Id: countdowntimer.module,v 1.8.2.22.2.4.2.26 2009/04/17 17:01:16 jvandervort Exp $
    ...
    Drupal.countdowntimer.formats = ['<em>(%dow% %moy%%day%)</em><br /><br/>%days% days + %hours%:%mins%:%secs%','Only %days% days, %hours% hours, %mins% minutes and %secs% seconds left', '%days% shopping days left', '<em>(%dow% %moy%%day%)</em><br /><br/>%days% days + %hours%:%mins%:%secs%'];

    Drupal.countdowntimer.jstimer = function(timer_element) {
      /* defaults */
      var d = {dir:"down",format_num:0, format_txt:"", timer_complete:new String('<em>In Progress</em>'), highlight:new String('style="color:red"').split(/=/), threshold:new Number('5')};
      /* jstimer.properties */
    ...
    Drupal.countdowntimer.parse_microformat = function() {
			...
			var cdt_class = $(this.element).children("span[@class=cdt_class]").html();
			if ( cdt_class == 'simple-timer' ) {
				this.d.tabclass = cdt_class;
				var interval = $(this.element).children("span[@class=cdt_interval]").html();
				var date = new Date();
				this.to_date = date;
				this.to_date.setTime(date.getTime() + (interval*1000));
			} else {
				this.d.tabclass = 'date-timer';
